...
export class ExpeditionRules extends MaterialRules<Color> {
  setup(): void {
    this.addItems(MaterialType.Card, places.map((place, i) => ({id: place, location: {type: LocationType.CardsDeck, x: i}})))
    /*material.shuffle(MaterialType.Place, LocationType.CardsDeck)
